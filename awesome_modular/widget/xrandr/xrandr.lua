local edid = require('widget.xrandr.edid')
...
function xrandr.actions.set_mode(name, mode)
  awful.spawn.with_shell(string.format('xrandr --output %s --mode %dx%d --rate %d', name, mode[1], mode[2], mode[3]))
end

function xrandr.actions.auto_mode(name)
  awful.spawn.with_shell(string.format('xrandr --output %s --auto', name))
end

function xrandr.actions.off(name)
  awful.spawn.with_shell(string.format('xrandr --output %s --off', name))
end

function xrandr.actions.set_rotate(name, rot)
  awful.spawn.with_shell(string.format('xrandr --output %s --rotate %s', name, rot))
end

function xrandr.actions.set_reflect(name, refl)
  awful.spawn.with_shell(string.format('xrandr --output %s --reflect %s', name, refl))
end

function xrandr.actions.set_relative_pos(name, relation, other)
  awful.spawn.with_shell(string.format('xrandr --output %s --%s %s', name, relation, other))
end

function xrandr.actions.set_primary(name)
  awful.spawn.with_shell(string.format('xrandr --output %s --primary', name))
end

function xrandr.actions.set_property(name, prop, value)
  awful.spawn.with_shell(string.format("xrandr --output %s --set '%s' '%s'", name, prop, value))
end
